...
bool compare_polar_lines(const Vec2f& l0, const Vec2f& l1) {
	return (l0[0] < l1[0]); // ascending (small values first)
}
...
int plancv(PlanData& data) {
	...
	Mat img_src, img_thr, img_edg, img_out, img_outw, img_outw2, img_outcc;
	...
	PCV_DEBUG(progress_msg("7.3 corners_connect..."));
	Frame frame;
	corners_connect2(linesw2, frame);
	cout << "nodes: " << frame.node_list().size() << ", edges: " << frame.edge_list().size();

	PCV_DEBUG(progress_msg("7.4 calculate connected components..."));
	list<list<ItCorner>> cc;
	frame.connected_components(cc);
