...
double angle2d(const Point2d& v) {
	Point2d x, y;
...
double angle_line(const Point2d& v) {
	Point2d x, y;
...
void getAandD(double& angle, double& distance, const Vec4i& line) {
	Point2d v(line[2] - line[0], line[3] - line[1]);	// direction vector of the line
	Point2d p(line[0], line[1]);						// support vector of line, i.e. f(t) = p + t * v
...
bool connectable(const line_t& l0, const line_t& l1) {
	...
	v1 = Point2f((float)(l0.ex - l0.sx), (float)(l0.ey - l0.sy));
	v1 /= fast_norm(v1);
	v2 = Point2f((float)(l1.ex - l1.sx), (float)(l1.ey - l1.sy));
	v2 /= fast_norm(v2);
	...
	assert(v1 + v2 != Point2f(0.0, 0.0));		// prevent division by 0
	assert(v1.ddot(v2) >= 0);
...
void lines_connect(vector<LineSeg>& lines, int max_gap, double max_dist) {
	...
	while (changes != 0) {
		...
		for(int i0 = 0; i0 < lines.size(); ++i0) {
			...
			for (int i1 = i0 + 1; i1 < lines.size(); ++i1) {
				...
				if (dist_ll(l0, l1) <= max_dist) {
					...
					v1 = Point2d((float)(l0.ex - l0.sx), (float)(l0.ey - l0.sy));
					v1 /= fast_norm(v1);
					v2 = Point2d((float)(l1.ex - l1.sx), (float)(l1.ey - l1.sy));
					v2 /= fast_norm(v2);
					...
					assert(v1 + v2 != Point2d(0.0, 0.0));		// prevent division by 0
					assert(v1.ddot(v2) >= 0);
		...
		PCV_DEBUG(cout << changes << endl);
	}

	PCV_DEBUG(cout << "->" << lines.size() << ", iterations: " << iteration_count << endl);
