...
int convex_hull4(const Vec2i** points) {
	...
	if (t[0] == 0) {
		...
		for (int k = 0; k < 4; ++k) {
			if (minc > points[k]->val[0]) {
				minc = points[k]->val[0];
				p0 = points[k];
			}
			if (maxc < points[k]->val[0]) {
				maxc = points[k]->val[0];
				p1 = points[k];
			}
		}
		...
		if (minc == maxc) {
			...
			maxc = -INT_MAX;
			for (int k = 0; k < 4; ++k) {
				if (minc > points[k]->val[1]) {
					minc = points[k]->val[1];
					p0 = points[k];
				}
				if (maxc < points[k]->val[1]) {
					maxc = points[k]->val[1];
					p1 = points[k];
				}
			}
...
double dist_ep4(const Vec2i* p0, const Vec2i* p1, const Vec2i* p2) {
	int64 dx = p1->val[0] - p0->val[0];
	int64 dy = p1->val[1] - p0->val[1];
	int64 a = p1->val[0] * p0->val[1] - p1->val[1] * p0->val[0];
	return (double)abs(dy * p2->val[0] - dx * p2->val[1] + a) / sqrt(dx * dx + dy * dy);
}

// Calculates the maximum distance from the given edge of a convex hull to the remaining point(s).
// The points are expected to be ordererd, i.e. it always calculates based on the edge given by p[0] and p[1].
double dist_ep4(const Vec2i* p0, const Vec2i* p1, const Vec2i* p2, const Vec2i* p3) {
	int64 dx = p1->val[0] - p0->val[0];
	int64 dy = p1->val[1] - p0->val[1];
	int64 a = p1->val[0] * p0->val[1] - p1->val[1] * p0->val[0];
	double norm = sqrt(dx * dx + dy * dy);

	double w0 = (double)abs(dy * p2->val[0] - dx * p2->val[1] + a) / norm;
	double w1 = (double)abs(dy * p3->val[0] - dx * p3->val[1] + a) / norm;
...
void min_linesegw(LineSegW& lw, const LineSeg& l0, const LineSeg& l1) {
	const Vec2i* hull[4] = {&l0.s, &l0.e, &l1.s, &l1.e};
	...
	if (hullsize == 1) {
		...
		Vec2f perp(-(float)(p1->val[1] - p0->val[1]), (float)(p1->val[0] - p0->val[0]));
		double norm = sqrt(perp[0] * perp[0] + perp[1] * perp[1]);
		perp *= (float)(0.5 * mind / norm);
		
		lw.s = Vec2i(	p0->val[0] + cvRound(perp[0]), 
						p0->val[1] + cvRound(perp[1]));
		lw.e = Vec2i(	p1->val[0] + cvRound(perp[0]), 
						p1->val[1] + cvRound(perp[1]));
		lw.width = (float)mind;
		...
		Vec2f v((float)(p1->val[0] - p0->val[0]), (float)(p1->val[1] - p0->val[1]));
		Vec2f u0((float)(p2->val[0] - p0->val[0]), (float)(p2->val[1] - p0->val[1]));
		Vec2f u1((float)(p3->val[0] - p0->val[0]), (float)(p3->val[1] - p0->val[1]));
		...
		lw.s = Vec2i(	p0->val[0] + cvRound(perp[0] + minv * v[0]),
						p0->val[1] + cvRound(perp[1] + minv * v[1]));
		lw.e = Vec2i(	p0->val[0] + cvRound(perp[0] + maxv * v[0]), 
						p0->val[1] + cvRound(perp[1] + maxv * v[1]));
		lw.width = (float)mind;
...
void test_convex_hull4() {
	...
	for (int l = 0; l < 10000; ++l) {
		...
		vector<cv::Vec2i> hull;
		vector<cv::Vec2i> points;
		points.push_back(Vec2i(rand()%S, rand()%S));
		points.push_back(Vec2i(rand()%S, rand()%S));
		points.push_back(Vec2i(rand()%S, rand()%S));
		points.push_back(Vec2i(rand()%S, rand()%S));
		const Vec2i* points2[5] = {&points[0], &points[1], &points[2], &points[3], (Vec2i*)0};
		...
		for (int k = 0; k < n; ++k) {
			...
			if (memcmp(&points3[0], &hull[0], n * sizeof(Vec2i)) == 0) {
				equal = 1;
				break;
			}
